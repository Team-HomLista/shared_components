...
export const PropertyMainInfo: FC<PropertyMainInfoProps> = ({ title, price, location, tags }) => {
    ...
    <div className="sm:pt-6 lg:pt-6.5">
          ...
          <Dialog open={isDialogOpen} onOpenChange={handleDialogOpenChange}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  ¿Te gusta esta propiedad? ¡Compártela con tus amigos y familiares!
                </DialogDescription>
              </DialogHeader>
              <div className="mt-4 flex flex-row items-center gap-2">
                <Card className="border-primary flex-1 truncate overflow-hidden border px-4 py-2 text-sm text-ellipsis select-all sm:text-base">
                  {isCopied ? "Se ha copiado el texto en el portapapeles." : url}
                </Card>
                <Button
                  variant="default"
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
        <h3 className="text-primary flex flex-row items-center gap-2 text-lg sm:text-xl">
          ...
          <span>{formatPrice({ value: price.current, currency: price.currency })}</span>
        </h3>
        <div className="flex flex-row items-center">
          <MapPinnedIcon className="text-primary h-4 w-4 sm:h-5 sm:w-5 lg:h-6 lg:w-6" />
          <p className="text-primary pl-1 text-sm sm:text-base lg:text-xl">{location}</p>
        </div>
      </div>
    </div>
  );
