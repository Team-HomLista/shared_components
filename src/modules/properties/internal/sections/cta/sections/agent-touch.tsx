...
export const AgentTouch: FC<AgentTouchProps> = ({ agent: { avatar, full_name: fullName } }) => {
  return (
    <div className="border-primary w-full rounded-lg border-2 p-4 sm:p-6 lg:w-xl">
      {/* header */}
      <div className="flex items-center gap-2 sm:gap-3">
        <img src={avatar} alt="Asesor" className="h-8 w-8 rounded-full sm:h-10 sm:w-10" />
        <div>
          <p className="text-sm sm:text-base">{fullName}</p>
        </div>
      </div>

      {/* description */}
      <p className="py-3 text-xs sm:py-4 sm:text-sm">
        Contacta con la agencia o asesor inmobiliario para recibir atención personalizada
      </p>
      ...
      <form className="space-y-3 sm:space-y-4">
        ...
        <p className="mt-2 text-center text-xs">
          Al enviar este formulario aceptas nuestras <Link href="#">Condiciones de uso.</Link>
        </p>
      </form>
    </div>
  );
