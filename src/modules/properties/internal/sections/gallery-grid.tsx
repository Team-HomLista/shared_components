 ...
 */
export const GalleryGrid: FC<GalleryGridProps> = ({ multimedia, onImageClick, onGalleryClick }) => {
      ...
      <div className="flex">
        <div className="grid h-[528px] w-full grid-cols-2 grid-rows-2 gap-2">
          ...
          {[1, 2].map((idx) => (
            <div
              key={idx}
              className="col-span-1 row-span-1 cursor-pointer overflow-hidden rounded-lg"
              onClick={() => onImageClick(idx)}
            >
              <img
                src={multimedia[idx]}
  ...
  return (
    <div className="flex">
      <div className="grid h-[528px] w-full grid-cols-2 grid-rows-2 gap-2">
        {/* Big image */}
        <div
          className="group relative col-span-1 row-span-2 cursor-pointer overflow-hidden rounded-lg"
          onClick={() => onImageClick(0)}
        >
          <img
            src={multimedia[0]}
          ...
          >
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/50 transition-colors group-hover:bg-black/60">
              <div className="text-center text-white">
                <div className="text-2xl font-bold">+{multimedia.length - 4}</div>
