// Adapted from https://github.com/aiji42/zod-i18n/blob/a747c683f02751e62983a7fbf82f8c20bc8bb6b9/packages/core/src/index.ts
...
export type MakeZodI18nMap = (i18n: i18n, option?: ZodI18nMapOption) => $ZodErrorMap;
...
export const makeZodI18nMap: MakeZodI18nMap = (i18n, option) => (issue) => {
  const { t, ns, handlePath } = {
    t: i18n.t as any as (...args: any[]) => any,
    ns: defaultNs,
    ...option,
    handlePath:
  ...
  switch (issue.code) {
    ...
    case "invalid_element":
      ...
      break;
    case "too_small": {
      const minimum = issue.origin === "date" ? new Date(issue.minimum as number) : issue.minimum;
      message = t(
    ...
    case "too_big": {
      const maximum = issue.origin === "date" ? new Date(issue.maximum as number) : issue.maximum;
    ...
    case "invalid_format":
      if (issue.format === "starts_with") {
        ...
        message = t("errors.invalid_format.ends_with", {
